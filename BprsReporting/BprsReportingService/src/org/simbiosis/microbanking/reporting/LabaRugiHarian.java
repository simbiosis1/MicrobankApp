...
public class LabaRugiHarian extends ReportServlet {
	...
	protected void onRequest(HttpServletRequest request)
		...
		//
		// Get config
		ConfigDto configCadangan = iSystemBp.getConfig(getSession(),
				"finReport.reserve");
		ConfigDto configTax = iSystemBp
				.getConfig(getSession(), "finReport.tax");
		ConfigDto configZakat = iSystemBp.getConfig(getSession(),
				"finReport.zakat");

		FinancialRpt rpt = null;
		Double value = 0D;
		if (configCadangan != null) {
			rpt = iFinReport.get(getCompany(), branch, date,
					configCadangan.getLongValue());
			value = rpt == null ? 0D : rpt.getEndValue();
			setParameter("Neraca.cadangan", value);
		}
		if (configTax != null) {
			rpt = iFinReport.get(getCompany(), branch, date,
					configTax.getLongValue());
			value = rpt == null ? 0D : rpt.getEndValue();
			setParameter("Neraca.pajak", value);
		}
		if (configZakat != null) {
			rpt = iFinReport.get(getCompany(), branch, date,
					configZakat.getLongValue());
			value = rpt == null ? 0D : rpt.getEndValue();
			setParameter("Neraca.zakat", value);
		}
	}
}
