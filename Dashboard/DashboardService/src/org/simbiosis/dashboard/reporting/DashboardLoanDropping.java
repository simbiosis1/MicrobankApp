...
@WebServlet("/getDashboardLoanDropping")
public class DashboardLoanDropping extends ReportServlet {
	...
	public DashboardLoanDropping() {
		super("DashboardLoanDropping");
	}
	...
	private List<LoanDroppingDv> prepareData() {
		...
		for (int i = 1; i <= cicle; i++) {
			...
			for (LoanTransInfoDto trans : transList) {
				...
				dv.setMargin(trans.getMargin() / 1000);
				result.add(dv);
			}
		}
		return result;
