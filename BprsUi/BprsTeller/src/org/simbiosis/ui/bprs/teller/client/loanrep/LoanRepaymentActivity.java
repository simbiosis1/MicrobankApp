...
import org.simbiosis.ui.bprs.teller.client.loanrep.ILoanRepayment.Activity;
...
public class LoanRepaymentActivity extends Activity {
	...
	public LoanRepaymentActivity(Place myPlace, AppFactory appFactory) {
	...
	public void start(AcceptsOneWidget panel, EventBus eventBus) {
		ILoanRepayment myForm = appFactory.getLoanRepaymentViewer();
	...
	private void onSave() {
		//
		showLoading();
		ILoanRepayment formEditor = appFactory.getLoanRepaymentEditor();
		...
		tellerSrv.saveSavingTrans(getKey(), data,
				new AsyncCallback<TransactionDv>() {
					...
					public void onSuccess(TransactionDv result) {
						hideLoading();
						ILoanRepayment myForm = appFactory
								.getLoanRepaymentViewer();
	...
	private void onNew() {
		...
		GetLoanHandler handler = new GetLoanHandler() {
			...
			public void showLoan(LoanDv loanDv) {
				ILoanRepayment myForm = appFactory.getLoanRepaymentEditor();
		...
		DlgGetLoan dialog = new DlgGetLoan(getKey(), handler);
		dialog.center();
	}
