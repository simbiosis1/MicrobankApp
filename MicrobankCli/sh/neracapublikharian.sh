...
DIRNAME=`dirname "$0"`
PROGNAME=`basename "$0"`
APP_JAR=CliFinancialReport.jar
APP_CLASS=org.simbiosis.cli.financial.CreatePublicReport
GREP="grep"
...
SIMBIOSIS="$APP_HOME/simbiosis/SystemDto.jar:$APP_HOME/simbiosis/MicrobankDto.jar:$APP_HOME/simbiosis/GlIntf.jar:$APP_HOME/simbiosis/GlBpIntf.jar:$APP_HOME/simbiosis/CliSimbiosis.jar"
CLASSPATH="$APP_HOME/$APP_JAR:$COMMON:$JACKSON:$JODATIME:$SIMBIOSIS"

# Execute the JVM in the foreground
eval \"$JAVA\" $JAVA_OPTS -cp "$CLASSPATH" $APP_CLASS `date +%d-%m-%Y`
APP_STATUS=$?
exit $APP_STATUS
