...
@WebServlet("/getNeracaPercobaanXls")
public class NeracaPercobaanXls extends ReportServlet {
	...
	public NeracaPercobaanXls() {
		super("NeracaPercobaanXls");
		setType(2);
	}
	...
	List<NeracaItemDv> prepareData() {
		...
		date2 = today.minusDays(1).toDate();
		...
		for (FinancialRpt dto : list) {
			...
			if (dto.getDebit() > ledger.getDebet()) {
				dto.setDebit(0);
			}
			if (dto.getCredit() > ledger.getKredit()) {
				dto.setCredit(0);
			}
			ledger.setDebet(ledger.getDebet() - dto.getDebit());
			ledger.setKredit(ledger.getKredit() - dto.getCredit());
